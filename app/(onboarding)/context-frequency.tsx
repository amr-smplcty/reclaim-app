...
export default function ContextFrequencyScreen() {
  const answers = useOnboardingStore((s) => s.answers);
  const updateAnswers = useOnboardingStore((s) => s.updateAnswers);
  const [selected, setSelected] = useState<string | null>(answers.frequencyNow);

  function handleNext() {
    updateAnswers({ frequencyNow: selected });
    goNextFrom('context-frequency');
  }
