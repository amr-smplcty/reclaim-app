...
**Note (resolved, not a deviation):** `bandColorToken` (`src/features/assessment/resultsVisual.ts`) previously colored PPCS-6 bands C/D with `destructive` (the pre-Driftwood behavior carried through the Epic 15 `danger` → `destructive` rename). This was **resolved in favor of Guardrail 2** ("no error-red for user behaviour"): a person's honest starting score is not an error state, and red at the results reveal is shame-adjacent framing that conflicts with the clinical design. C/D now use `caution`; the band label and its plain-language meaning carry the distinction from B, and `destructive` stays reserved for irreversible data actions. (BACKLOG #64, resolved.)
