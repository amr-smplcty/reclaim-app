...
4b. **Earn-in daily stakes (v1.2 upgrade):** at goal setup, user sets an affordable monthly amount and names a concrete reward they're saving for; the amount divides into daily stakes. Completing the day's session moves that day's stake INTO the reward jar; a missed day's stake is never earned (v1.2: tracked pledge; v2 real money: the missed day's stake goes to a user-chosen charity from a curated generic list). The fork happens at the day level, before earning — the jar itself remains strictly monotonic and nothing earned is ever confiscated (write-off-effect protection). Stakes bind ONLY to verifiable session completion, never to self-reported clean days. Anti-charity variants are deferred indefinitely (brand/clinical-tone risk).
