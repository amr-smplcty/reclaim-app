...
| 35 | Push notification *scheduling* for daily lesson/evening check-in/risky-window/re-assessment reminders (PRODUCT_SPEC §7) is still unbuilt. Epic 9 added where the user's chosen daily-lesson and evening-check-in times are stored (Settings → Notifications, `useSettingsStore`), and Epic 7 built the re-assessment due-date in-app mechanism (`isReassessmentDue`, Progress tab banner) — but nothing actually schedules a local/push notification anywhere yet | Epic 7 report / Epic 9 report | next notifications epic |
