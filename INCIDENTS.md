...
- **Verified (fresh session):** Re-tested `npx --version`/`npm --version` in a brand-new session — both resolve cleanly. Confirmed the `env.PATH` block is still the thing making this work, not some other machine-level change: `/usr/local/bin` has no node/npm/npx symlinks (only unrelated Parallels/WorkspaceONE entries), the real binaries live only at `/opt/homebrew/bin/{node,npm,npx}`, and a fully clean shell (`env -i zsh -c 'echo $PATH'`) yields `/bin:/usr/bin:/usr/ucb:/usr/local/bin` — no Homebrew path — because there's no `~/.zshenv` to source `brew shellenv` for non-interactive shells (only `~/.zprofile`, which non-interactive shells skip). So `.claude/settings.json`'s `env.PATH` remains the active, load-bearing fix; keep the block.
- **Prevention rule:** **If a standard binary is "not found" in the Bash tool, fix the environment (`.claude/settings.json`'s `env.PATH`, or ask the user to) — never paper over it with an `export PATH=` prefix or an absolute-path substitute on every command. Both defeat permission pre-approval and are themselves the problem, not a workaround for it. Before crediting a different fix (e.g. binary symlinks) in a write-up, verify it actually exists on disk — don't take a remembered/reported cause at face value.**

### INC-16 · EAS iOS build failed on `EXAV`/`ExpoModulesCore` header — expo-av removed in SDK 54
- **Symptom:** the first EAS iOS build (Epic 3 dev client) failed native compilation: `'ExpoModulesCore/EXEventEmitter.h' file not found` / `could not build Objective-C module 'EXAV'`. Every JS-level gate had been green — typecheck, 563 tests, all three custom lints, and `verify:bundle` (a real Metro dev-mode export) all passed — yet the native build broke.
- **Root cause:** `expo-av` was removed from Expo in **SDK 54**; this project is on **SDK 57**, so `expo-av`'s native module (`EXAV`) can no longer compile against the current `ExpoModulesCore` headers. It sat in `package.json` as a **dead dependency** — grep confirmed zero imports of `expo-av` anywhere in `src/`/`app/` (the Urge Surf and Breather tools were always visual-only; their guided audio is TODO(content), BACKLOG #4). `verify:bundle` can't catch this class of failure by construction: it exports the *JS* bundle via Metro and never compiles native modules, so an unused-but-uncompilable native package passes every JS gate and only fails when a real device/simulator build links native code. `expo-doctor` was never part of the loop, so the SDK-incompatibility warning it would have raised went unseen. (Same session also surfaced 28 packages drifted off their SDK-57 expected versions, and three other installed-but-unused native deps — `@shopify/react-native-skia`, `victory-native`, `@expo/ui` — all removed to shrink native build surface.)
...
### INC-18 · Content under the status bar + undismissable keyboards — layout/interaction bugs no JS gate can catch
- **Symptom:** first physical-device test (dev build on iPhone) surfaced two problems every automated gate had passed clean: (1) screen content rendered *under* the status bar — the system clock overlapped the onboarding section header and the top of content was clipped; (2) on any text-input screen the keyboard covered content with no way to dismiss it (no Done button, no tap-to-dismiss).
- **Root cause:** (1) the app had `react-native-safe-area-context` installed but used *nowhere* — no `SafeAreaProvider`, no insets. Native-header screens (tabs/program/toolkit) happened to be fine because React Navigation's header insets the top, but the headerless screens (onboarding `headerShown: false`, modals) rendered full-bleed under the status bar. (2) No keyboard handling existed anywhere — no `KeyboardAvoidingView`, no dismiss affordance, no `keyboardShouldPersistTaps`. **Neither is detectable by any JS-level or export-level check**: unit tests / typecheck assert logic and types; `verify:bundle` only proves the JS bundle *builds*; `expo-doctor` only checks package/SDK compatibility. None of them render pixels or drive a keyboard, so a screen can pass every gate and still be visually broken or unusable on a device.
- **Fix:** `SafeAreaProvider` at the root; a themed `SafeAreaScreen` wrapper applied to every headerless screen (onboarding via `OnboardingLayout`, all modals with edges chosen per presentation — full-screen onboarding `top+bottom`, card modals `bottom`); a global `<KeyboardToolbar/>` (iOS "Done" bar, mounted once — covers every input with no per-input wiring); `KeyboardAvoidingView` + `keyboardShouldPersistTaps="handled"` (tap-outside-dismiss) + `keyboardDismissMode` on the input screens (via a `KeyboardAwareScrollView` helper).
- **Prevention rule:** **A device visual pass is a required gate before any TestFlight build — specifically covering (a) safe areas: no screen renders under the status bar or home indicator, on every screen type (onboarding, tabs, modals, toolkit, lesson/exercise, journal, progress, settings, paywall, emergency card); and (b) every text input: the keyboard can be dismissed (Done + tap-outside) and never covers the focused field or its primary CTA. JS gates (tests, typecheck, verify:bundle, expo-doctor) structurally cannot catch layout or keyboard-interaction defects — only viewing on a real device can.**
...
17. A device visual pass (safe areas on every screen type + keyboard dismiss/coverage on every text input) is required before TestFlight — JS gates can't catch layout or keyboard-interaction bugs (INC-18).
